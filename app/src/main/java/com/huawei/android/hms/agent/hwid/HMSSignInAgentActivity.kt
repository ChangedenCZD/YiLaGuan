 ...
 */
class HMSSignInAgentActivity : BaseAgentActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        val signInResult = SignInApi.INST.signInResult
        if (signInResult != null) {
            try {
                val nxtIntent = signInResult.data
                val statusCode = signInResult.status.statusCode
                var nxtRequestCode = 0
                ...
                HMSAgentLog.e("start activity error:" + e.message)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent) {
        ...
        if (requestCode == REQUEST_SIGN_IN_UNLOGIN || requestCode == REQUEST_SIGN_IN_CHECK_PASSWORD) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                val result = HuaweiId.HuaweiIdApi.getSignInResultFromIntent(data)
                ...
                if (result == null) {
                    ...
                    SignInApi.INST.onSignInActivityResult(result.status.statusCode, result.signInHuaweiId, false)
                } else {
                    // 授权失败，result.getStatus()获取错误原因
                    HMSAgentLog.e("授权失败 失败原因:" + result.status.toString())
                    SignInApi.INST.onSignInActivityResult(result.status.statusCode, null, false)
                }
            } else {
    ...
    companion object {
        ...
        private val REQUEST_SIGN_IN_UNLOGIN = 1002
        private val REQUEST_SIGN_IN_AUTH = 1003
        private val REQUEST_SIGN_IN_CHECK_PASSWORD = 1005
    }
}
